    ...
    def update(self, states, actions, rewards, next_states, dones, log_probs, values):
        ...
        for _ in range(self.epochs):
            for start in range(0, self.batch_size, self.mini_batch_size):
                ...
                mini_batch_indices = slice(start, end)
                ...
                mini_batch_old_values = old_values[mini_batch_indices]

                action_logits, state_values = self.model(mini_batch_states, return_value=True)
                new_log_probs = torch.distributions.Categorical(torch.softmax(action_logits, dim=-1)).log_prob(mini_batch_actions)
                ...
                surrogate2 = torch.clamp(ratio, 1 - self.clip_epsilon, 1 + self.clip_epsilon) * mini_batch_advantages
                policy_loss = -torch.min(surrogate1, surrogate2).mean()

                # Value function clipping
                values_clipped = mini_batch_old_values + (state_values - mini_batch_old_values).clamp(-self.clip_epsilon, self.clip_epsilon)
                value_loss1 = (state_values - mini_batch_returns).pow(2)
                value_loss2 = (values_clipped - mini_batch_returns).pow(2)
                ...
                approx_kl_div = ((mini_batch_old_log_probs - new_log_probs) ** 2).mean().item()
