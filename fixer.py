...
class WarehouseEnvironment:
    ...
    def reset(self):
        ...
        local_obs, local_map, _, _, _, _, _, _, _, _, _ = update_coords(
            self.dynamic_coords, self.init_arr, self.agent_idx, self.time_idx,
            self.local_fov, self.global_mapper_arr, [0,0], self.agent_prev_coord,
            self.cells_skipped, self.dist, self.agent_path[-1]
        )
        
        target_array = (2*self.local_fov, 2*self.local_fov, 4)
        combined_arr = np.dstack((local_obs, local_map.reshape(local_map.shape[0], local_map.shape[1], 1)))
        combined_arr = symmetric_pad_array(combined_arr, target_array, 255)
        
        # Initialize the observation buffer with 4 copies of the initial observation
        for _ in range(4):
            self.observation_history.append(combined_arr)
        
        # Create the initial 5D tensor
        stacked_obs = np.stack(list(self.observation_history), axis=0)
        stacked_obs = stacked_obs.reshape(1, 4, stacked_obs.shape[1], stacked_obs.shape[2], stacked_obs.shape[3])
        ...
        return start[0] * start[1], stacked_obs
    ...
    def step(self, action):
        ...
        if len(local_obs) > 0:
            ...
            # Create the 5D tensor with the last 4 observations
            stacked_obs = np.stack(list(self.observation_history), axis=0)
            
            # Ensure we have 4 observations (pad with copies of the first observation if necessary)
            if stacked_obs.shape[0] < 4:
                padding = [(0, 4 - stacked_obs.shape[0])] + [(0, 0) for _ in range(stacked_obs.ndim - 1)]
                stacked_obs = np.pad(stacked_obs, padding, mode='edge')
            
            # Reshape to (1, 4, H, W, C)
            stacked_obs = stacked_obs.reshape(1, 4, stacked_obs.shape[1], stacked_obs.shape[2], stacked_obs.shape[3])
        else:
            stacked_obs = np.array([])

        return_values = (stacked_obs, self.agent_prev_coord[0] * self.agent_prev_coord[1], rewards, isAgentDone)
