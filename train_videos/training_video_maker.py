...
def create_video_from_images(image_folder, train_number, video_name='train_x_video.avi', fps=5, codec='MJPG', scale=50):
    ...
    images.sort(key=lambda x: int(re.search(r'train_\d+_(\d+).png', x).group(1)))
...
create_video_from_images(f'G2RL-Path-Planning/training_images/train_{training_index}_images', training_index, f'train_{training_index}_video.avi', fps=7, codec='MJPG', scale=10)
