...
def rollout(policy, env, viewer):
	...
	while True:
		...
		while not done:
			...
			# Viewer environment if specified, off by default
			if viewer:
				viewer.sync()
...
def eval_policy(policy, env, viewer=False):
	"""
	...
	for ep_num, (ep_len, ep_ret) in enumerate(rollout(policy, env, viewer)):
